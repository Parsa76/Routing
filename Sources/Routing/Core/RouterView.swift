...
public struct RouterView<Content: View>: View, Router {
    ...
    @State var addNavigationView: Bool = true
    ...
    public init(
     ...
     public var body: some View {
        NavigationStackIfNeeded(path: $path, addNavigationView: addNavigationView) {
            content(self)
                .sheetViewModifire(screen: $showSheet)
                .fullScreenCoverViewModifire(screen: $showFullScreenCover)
                .showCustomAlert(type: alertOption, alert: $alert)
            
        }
        .modalViewModifire(backgroundColor: modalBackgroundColor, transition: modalTransition, screen: $modal)
        .environment(\.router, self)
    }
    
    public func showScreen<T: View>(_ option: SegueOption, @ViewBuilder destination: @escaping (Router) -> T) {
        let screen = RouterView<T>(
            screenStack: option.shouldAddNavigationView ? nil : screenStack.isEmpty ? $path : $screenStack,
            addNavigationView: option.shouldAddNavigationView
        ) { newRouter in
    ...
    public func showAlert(_ option: AlertType, title: String? = nil, subtitle: String? = nil,buttons: (@Sendable () -> AnyView)? = nil) {
